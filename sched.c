...
static bool irq_is_enabled = true;
...
void irq_disable(void) {
    if (irq_is_enabled) {
        sigset_t filled_sigmask;
        sigfillset(&filled_sigmask);
        sigprocmask(SIG_SETMASK, &filled_sigmask, &sigmask);
        irq_is_enabled = false;
    }
}
...
void sched_cont(void (*entrypoint)(void *aspace), void *aspace, int timeout) {
    if (current->next != current) {
        fprintf(stderr, "Multiple sched_cont\n");
        return;
    }
...
void sched_time_elapsed(unsigned amount) {
    ...
    while (time < endtime && action.sa_handler == (__sighandler_t) tick_hnd) {
        sigsuspend(&sigmask);
        sigaction(SIGALRM, NULL, &action);
    }
    irq_enable();
...
static void tick_hnd(void) {
    time += 1;
    while (waitq != NULL && waitq->waketime <= time) {
        struct task *t = waitq;
        waitq = waitq->next;
        policy_run(t);
    }
