...
void irq_disable(void) { sigprocmask(SIG_BLOCK, &irqs, NULL); }

void irq_enable(void) { sigprocmask(SIG_UNBLOCK, &irqs, NULL); }
...
static void vmctx_apply(struct vmctx *vm) {
  ...
  for (int i = 0; i < USER_PAGES; ++i) {
    ...
    void *addr = mmap(USER_START + i * PAGE_SIZE, PAGE_SIZE,
                      PROT_READ | PROT_WRITE | PROT_EXEC,
                      MAP_SHARED | MAP_FIXED, memfd, vm->map[i] * PAGE_SIZE);
    if (addr == MAP_FAILED) {
      perror("mmap");
      abort();
    }
...
struct task *sched_new(void (*entrypoint)(void *aspace), void *aspace,
                       int priority) {
...
static int fifo_cmp(struct task *t1, struct task *t2) { return -1; }
...
static void top(int sig, siginfo_t *info, void *ctx) {
  ...
  hctx_push(regs, sig);
  hctx_push(regs,
            (unsigned long)(current->stack + sizeof(current->stack) - 16));
...
static void syscallbottom(unsigned long sp) {
  ...
  if (sc->rax == os_syscall_nr_fork) {
    ...
    sc->rax = syscall_do(sc->rax, sc->rbx, sc->rcx, sc->rdx, sc->rsi,
                         (void *)sc->rdi);
  }
}
...
int sys_exec(const char *path, char **argv) {
  ...
  if (!ehdr->e_phoff || !ehdr->e_phnum || !ehdr->e_entry ||
...
static void copyrange(struct vmctx *vm, unsigned from, unsigned to) {
  for (unsigned i = from; i < to; ++i) {
    ...
    if (-1 == pwrite(memfd, USER_START + i * PAGE_SIZE, PAGE_SIZE,
                     vm->map[i] * PAGE_SIZE)) {
      perror("pwrite");
      abort();
    }
...
int sys_close(int fd) { set_fd(current, fd, NULL); }
