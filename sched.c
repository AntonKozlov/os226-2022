...
sigset_t *get_sig_alrm(void) {
	static sigset_t *sig_m = NULL;
	if (sig_m != NULL)
		goto out_ss;
        sigemptyset(sig_m); 
        sigaddset(sig_m, SIGALRM); 
        out_ss:
        return sig_m;
}

void irq_disable(void) {
        sigprocmask(SIG_BLOCK, get_sig_alrm(), NULL); 
}

void irq_enable(void) {
        sigprocmask(SIG_UNBLOCK, get_sig_alrm(), NULL);
}
